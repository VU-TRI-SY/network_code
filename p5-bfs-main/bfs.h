...
typedef struct {          // SuperBlock
  ...
  i16 firstFree;          // DBN of first free block - stores the dbn of the first free block
} Super;
...
typedef struct {          // Inode
  ...
  //indirect stores the dbn of a block that stores dbns for 256 entries
} Inode;



typedef struct {          // Dir
  char fname[NUMINODES][FNAMESIZE]; //array of string: string fname[NUMINODES]
} Dir; 


typedef struct {          // Open File Table Entry
  i32 inum;               // inum of file. O => slot not used
  i32 refs;               // # processes fsOpen'd this file - the number of processes that refer to this entry
  i32 curs;               // cursor into file //refer to the working position
} OFTE;
