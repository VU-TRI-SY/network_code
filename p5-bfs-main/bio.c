...
i32 bioWrite(i32 dbn, void* buf) {
  ...
  i32 boff = dbn * BYTESPERBLOCK; //byte offset of block dbn
  i32 ret  = fseek(fp, boff, SEEK_SET); //move fp pointer to the specified position
  if (ret != 0) { fclose(fp); FATAL(ret); }

  i32 numb = fwrite(buf, 1, BYTESPERBLOCK, fp); //write content of buff to file, write 512 elements, each element has size 1 byte
  //fwrite: return number of written bytes
