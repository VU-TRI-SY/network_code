...
i32 bfsInitFreeList() {
  ...
  //each element of buff has size 2 bytes --> we use a block (512 bytes) for buff --> buff size = 256
  i32 ret = 0; //int = i32 = 4 bytes

  for (int dbn = NUMMETA; dbn < BLOCKSPERDISK - 1; ++dbn) { //for initing, the first 3 blocks used for superblock, inodes, dir
  //--> then the free list must contain blocks from dbn3
  //for: run from the first free block(block dbn3) to the neighbor of the last free block(block with dbn99)
  // run from 3 to 98
    buf[0] = dbn + 1; //use the first 2 bytes to stores the dbn of next block
    bioWrite(dbn, (i8*)buf); //write content of buf for the 'dbn' block (write content for the this free block)
  }

  buf[0] = 0; //the last block links to null
...
i32 bfsInitDir() {
  ...
  i8 buf[BYTESPERBLOCK] = {0}; //buf has size 512 and each element has size 8 bits ~ 1 bytes, all set to 0
  return bioWrite(DBNDIR, buf); //set dbn 2 all to 0
}
...
i32 bfsInitInodes() {
  // if (fp == NULL) FATAL(ENULLPTR);
  i8 buf[BYTESPERBLOCK] = {0};
  return bioWrite(DBNINODES, buf); //set dbn 1 all to 0
}
...
i32 bfsInitOFT() {
  for (i32 i = 0; i < NUMOFTENTRIES; ++i) {
    g_oft[i].inum = 0; //inum = 0 --> slot not used
    g_oft[i].curs = 0;
    g_oft[i].refs = 0;
  }
  return 0;
...
i32 bfsInitSuper(FILE* fp) {
  ...
  i8 buf[BYTESPERBLOCK] = {0};
  memcpy(buf, &sb, sizeof(Super)); //copy data of sb to buff
...
// ============================================================================
i32 bfsRead(i32 inum, i32 fbn, i8* buf) { //given inum for a file and fbn is block to read
                                          //read content from block fbn and store to buff
...
// ============================================================================
i32 bfsTell(i32 fd) { //pass a file descriptor 'fd' --> return the position of file pointer ~ cursor
